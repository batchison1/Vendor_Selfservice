...
```
vss/
  backend/    ASP.NET Core 10 Web API + EF Core (SQLite) + ERP stub + xUnit tests
...
- Creates + seeds a local SQLite DB (`Vss.Api/vss.db`) on first run.
...
Run the tests (they boot the API against SQLite in-memory — a real relational provider):
...
- `dotnet test` — 4/4 passing (link → read → change → admin approve → ERP push) on SQLite.
- `npm run build` / `npm run typecheck` — clean.
- Live click-through (dev auth + local backend): link → console → edit Banking → submit;
  admin approval pushes the change to the ERP stub and updates the record.
