...
export function AuthLayout({ children }: { children: ReactNode }) {
  return (
    <div style={{ minHeight: "100vh", display: "grid", gridTemplateColumns: "1.1fr 1fr" }}>
      {/* brand panel */}
      <div style={{ position: "relative", background: "linear-gradient(150deg,#0C2F50 0%,#0C2F50 45%,#17516B 100%)", color: "#fff", padding: "56px 64px", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ position: "relative", display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, letterSpacing: ".14em", textTransform: "uppercase", color: "#7AC8CC" }}>Vendor Self Service</div>
          </div>
        </div>
        <div style={{ position: "relative", marginTop: "auto" }}>
          <div style={{ fontSize: 12, letterSpacing: ".16em", textTransform: "uppercase", color: "#7AC8CC", fontWeight: 600 }}>City of Bozeman · Supplier Portal</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14, marginTop: 32 }}>
            {POINTS.map((p) => (
              <div key={p} style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 14, color: "rgba(255,255,255,.9)" }}>
                <span style={{ width: 22, height: 22, borderRadius: 999, background: "rgba(52,167,173,.28)", color: "#7AC8CC", display: "flex", alignItems: "center", justifyContent: "center", flex: "0 0 22px" }}>
